...
class GraphCLIPOutput(ModelOutput):
    ...
    Attributes:
        ...
        logits_image_graph (torch.FloatTensor): Logits for image-graph pairs.
        logits_graph_text (torch.FloatTensor): Logits for graph-text pairs.
    ...
    logits_image_graph: torch.FloatTensor
    logits_graph_text: torch.FloatTensor
...
class GraphCLIPModel(CLIPModel):
    ...
    def forward(
        ...
        logits_image_graph = logit_scale * torch.matmul(image_embeds, graph_embeds.t())
        logits_graph_text = logit_scale * torch.matmul(graph_embeds, text_embeds.t())

        loss = None
        if return_loss:
            # Compute contrastive loss for all pairs and average
            loss_image_text = clip_loss(logits_image_text)
            loss_image_graph = clip_loss(logits_image_graph)
            loss_graph_text = clip_loss(logits_graph_text)
            loss = (loss_image_text + loss_image_graph + loss_graph_text) / 3
        ...
        return GraphCLIPOutput(
            ...
            graph_embeddings=graph_embeds,
            logits_image_graph=logits_image_graph,
            logits_image_text=logits_image_text,
            logits_graph_text=logits_graph_text,
        )
